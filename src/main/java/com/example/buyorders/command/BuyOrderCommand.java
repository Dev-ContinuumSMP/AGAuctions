...
public class BuyOrderCommand implements CommandExecutor, TabCompleter {
    ...
    public BuyOrderCommand(BuyOrders plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!handMode && args.length < 3) {
            player.sendMessage(BuyOrders.color("&eUsage: /buyorder <material> <amount> <priceEach>"));
            return true;
        }

        if (handMode && args.length < 3) {
            player.sendMessage(BuyOrders.color("&eUsage: /buyorder hand <amount> <priceEach>"));
            return true;
        }
        ...
        if (handMode) {
            ...
            if(hand == null || hand.getType().isAir()) {
                player.sendMessage(BuyOrders.color("&cHold the custom item you want to order."));
                return true;
            }
        ...
        if (plugin.getOrderManager().getOrdersByPlayer(player.getUniqueId()).size() >= maxOrders) {
            player.sendMessage(BuyOrders.color("&cYou already have &e" + maxOrders + " &copen buy orders."));
            return true;
        }
